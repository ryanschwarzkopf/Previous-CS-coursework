...
public class IntArrayBag implements Cloneable
{
   ...
   **/   
   ...
   /**
    * Method returns true if b and the bag that activates the method have exactly 
    * the same number of every element. Otherwise, the method returns false.
    * 
    * @author Ryan Schwarzkopf
    * 
    * @param b
    * 	IntArrayBag, assumed to not be a null pointer
    * @return
    * 	true if b and current job are equal
    */
   public boolean equals(IntArrayBag b) {
	   ...
	   for(int i=0;i<manyItems;i++) {
		   x = this.countOccurrences(data[i]);
		   y = b.countOccurrences(data[i]);
		   if(x!=y) same=false;
	   }
